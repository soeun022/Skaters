function startApp() {
    ...
    function updateActiveMonthFromScroll() {
        ...
        let targetBlock = blocks.find(b => {
            ...
            return rect.top <= Y_TARGET && rect.bottom > Y_TARGET;
        });

        if (!targetBlock) {
            const firstRect = blocks[0].getBoundingClientRect();
            if (firstRect.top > Y_TARGET) {
        ...
        if (targetBlock) {
            const yearStr = targetBlock.dataset.year;
            const monthStr = targetBlock.dataset.month;
            const title = targetBlock.dataset.title;
